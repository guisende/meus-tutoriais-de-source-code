...
class PlayState extends MusicBeatState
{
	...
	//attack things
	var attackTimer:Float = 5; //TEMPO PRA ATACAR
	var attackDamage:Float = 1; //A PRA CURA DE VIDA SE DEIXAR 1 E SE VOCE ATACAR ELE CURA INTERINHO
	...
	override public function update(elapsed:Float)
	{
		...
		if (curSong.toLowerCase() == "bopeebo"){ //ONDE TA NOME BOPEEBO E O NOME DA SUA MUSICA
			ableToAttack = true;
			if (FlxG.keys.justPressed.SHIFT && ableToAttack && !attacking){
				boyfriend.playAnim('pre-attack', true); //ANIMAÇAO PRA ANTES ATACAR CHECA ISSO NO BF.JSON
				attacking = true;
				ableToAttack = false;
				dad.playAnim('singUp', true); //ANIMAÇAO DO OPONENTE
				health += attackDamage;
				new FlxTimer().start(0.2, function(tmr:FlxTimer){
				boyfriend.playAnim('attack', true); //ANIMAÇAO DO BF ATACANDO CHECA ISSO NO BF.JSON
				});
